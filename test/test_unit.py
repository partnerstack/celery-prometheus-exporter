...
class TestBucketLoading(TestCase):
    ...
    def test_from_env(self):
        ...
        self.assertEqual(
            [1.0, 2.0, 3.0], get_histogram_buckets_from_evn("TEST_BUCKETS")
        )
...
class TestMockedCelery(TestCase):
    ...
    def test_tasks_events(self):
        ...
        self.assertAlmostEqual(
            REGISTRY.get_sample_value("celery_task_latency_sum"), latency_before_started
        )
        assert (
            REGISTRY.get_sample_value(
                "celery_tasks_runtime_seconds_count", labels=dict(name=self.task)
            )
            == 1
        )
        assert (
            REGISTRY.get_sample_value(
                "celery_tasks_runtime_seconds_sum", labels=dict(name=self.task)
            )
            == 234.5
        )
    ...
    def test_can_measure_queue_length(self):
        ...
        sample = REGISTRY.get_sample_value(
            "celery_queue_length", {"queue_name": "realqueue"}
        )
    ...
    def test_set_zero_on_queue_length_when_an_channel_layer_error_occurs_during_queue_read(
        ...
        sample = REGISTRY.get_sample_value(
            "celery_queue_length", {"queue_name": "noqueue"}
        )
    ...
    def _assert_task_states(self, states, cnt):
        for state in states:
            assert (
                REGISTRY.get_sample_value("celery_tasks", labels=dict(state=state))
                == cnt
            )
            task_by_name_label = dict(state=state, name=self.task)
            assert (
                REGISTRY.get_sample_value(
                    "celery_tasks_by_name", labels=task_by_name_label
                )
                == cnt
            )
