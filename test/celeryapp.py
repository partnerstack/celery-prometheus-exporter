...
celery_app.conf.update(
    ...
    CELERY_QUEUES=(
        Queue(
            "queue1", exchange=Exchange("queue1", type="direct"), routing_key="queue1"
        ),
        Queue(
            "queue2", exchange=Exchange("queue2", type="direct"), routing_key="queue2"
        ),
        Queue(
            "queue3", exchange=Exchange("queue3", type="direct"), routing_key="queue3"
        ),
    ),
    CELERY_ROUTES={
